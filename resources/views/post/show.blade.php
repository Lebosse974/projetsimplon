...
<div class="px-4 my-4 bg-white rounded-lg shadow-md ">
    <div class="flex flex-col border-b border-black ">
        <div class="flex items-center">
            ...
            <a href="{{route('communaute.show',$post->communautes->id)}}" class="hidden px-2 font-bold text-black lg:block sm:text-xs lg:text-base">{{$post->communautes->name}}</a>
...
<div class="px-4 my-4 bg-white rounded-lg shadow-md ">
    <div class="flex flex-col border-b border-black ">
        <div class="flex items-center">
            ...
            <p class="text-sm font-bold text-black lg:text-base"> {{ $post->users->pseudo }}. <span class="text-gray-500">{{ $post->created_at->format('Y-m-d ') }},  {{ $post->created_at->format('H') }} H {{ $post->created_at->format('i') }}</span></p>
        </div>
        <h1 class="text-xl font-bold text-gray-700 md:text-2xl"> {{ $post->title }}</h1>
        <p class="text-sm text-justify lg:text-base">{{ $post->content }}</p>
